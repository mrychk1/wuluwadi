...
export function floodEvolution(map: mars3d.Map) {
  ...
  graphicLayer.on(mars3d.EventType.click, function (event) { // 监听图层点击事件
    const pickedItem = event.pickedObject?.data; // 获取点击的对象数据
    console.log("单击了合并对象中的单个值为", pickedItem); // 打印点击的对象数据
  });
  colorRamp = new mars3d.ColorRamp({ // 实例化颜色渐变
    steps: [0, 0.5, 1, 2], // 定义颜色渐变的步骤
    colors: ["#BDD5F6", "#A2C4F4", "#6E9FF1", "#668FD2"], // 定义颜色渐变的颜色
  });
  let progressValue = 0; // 初始化当前进度为0
  const intervalId = setInterval(async () => { // 设置定时器，每50毫秒执行一次
    progressValue++; // 进度加1
    if (progressValue < 578) { // 如果进度小于578
      await loadAndRenderGeoJSON(progressValue); // 加载并渲染GeoJSON数据
    } else { // 如果进度达到或超过578
...
function manageFloods(graphic: mars3d.graphic.PolygonCombine) {
  if (floods.length > 1) { // 如果洪水数据数组长度大于1
